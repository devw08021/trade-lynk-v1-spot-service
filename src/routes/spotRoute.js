...
spotRoute.get("/order-book/:pairId", (c) => c.json({ message: "Order book endpoint" }));
spotRoute.get("/open-order/:pairId", authMiddleware, (c) => spotController.getOpenOrder(c));
spotRoute.get("/filled-order/:pairId", authMiddleware, (c) => spotController.getFilledOrder(c));
spotRoute.get("/order-history/:pairId", authMiddleware, (c) => spotController.getOrderHistory(c));
spotRoute.get("/trade-history/:pairId", authMiddleware, (c) => spotController.getTradeHistory(c));
...
spotRoute.post("/cancel-order", authMiddleware, (c) => c.json({ message: "Cancel order endpoint" }));

// Depth chart (not implemented yet due to missing deps)
spotRoute.post("/depth-chart", (c) => spotController.getDepthData(c));

// Order History routes
spotRoute.get("/get-my-spot-history", authMiddleware, (c) => c.json({ message: "My spot history endpoint" }));
spotRoute.get("/get-filled-order-history", authMiddleware, (c) => c.json({ message: "Filled order history endpoint" }));

// Chart routes
spotRoute.get("/chart/:config", (c) => c.json({ message: "Chart data endpoint" }));
spotRoute.post("/chart-update-to-db", (c) => c.json({ message: "Chart update to DB endpoint" }));
spotRoute.post("/chart-update-to-redis", (c) => c.json({ message: "Chart update to Redis endpoint" }));
